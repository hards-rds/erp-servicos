...
type EnrollmentRow = { id: string; starts_at: string; due_day: number; monthly_amount: number; discount_amount: number; status: string; school_athletes: Relation<{ full_name: string }>; school_classes: Relation<{ name: string; category: string }>; school_guardians: Relation<{ full_name: string; client_id: string | null }> };
...
export default async function EnrollmentsPage({ searchParams }: PageProps) {
  ...
  const { data } = context.allowed && companyId ? await context.supabase.from("school_enrollments").select("id,starts_at,due_day,monthly_amount,discount_amount,status,school_athletes(full_name),school_classes(name,category),school_guardians(full_name,client_id)").eq("company_id", companyId).order("created_at", { ascending: false }) : { data: [] };
