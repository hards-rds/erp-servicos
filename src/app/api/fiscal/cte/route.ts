...
export async function POST(request: NextRequest) {
  ...
  const taxData = { cst: value(data, "taxCst") || null, baseAmount: num(data, "taxBaseAmount", 0), rate: num(data, "taxRate", 0), amount: num(data, "taxAmount", 0) };
  ...
  const { data: document } = await supabase.from("cte_documents").select("id,cfop,operation_nature,issue_state,freight_value,client_id,transport_trips!cte_documents_trip_fk(origin_city_code,destination_city_code,transport_vehicles!transport_trips_vehicle_fk(plate),transport_drivers!transport_trips_driver_fk(document)),clients!cte_documents_client_fk(document)").eq("company_id", profile.company_id).eq("id", cteId).maybeSingle();
